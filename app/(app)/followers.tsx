...
export default function FollowersScreen() {
  ...
  useEffect(() => {
    getFollowDetails(id).then((data) => setFollowers(data.followers)).catch(() => {});
  }, [id]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: insets.top + 16, paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 0.5, borderColor: isDark ? '#333' : '#eee' }}>
        ...
        <Text style={{ color, fontFamily: 'jakaraBold', fontSize: 18 }}>Followers</Text>
      </View>
      <FlatList
        data={followers}
        keyExtractor={(item, i) => item.follower_id ?? i.toString()}
